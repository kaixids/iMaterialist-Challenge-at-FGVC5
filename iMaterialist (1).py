...
# Stacking the input arrays
train_input = np.vstack(train_data)
# Saving it
np.save('train_input_stacked', train_input)


# Gathering the list of labels for the training images
train_labels = []
...
# Turning the list into binary classifier 
mlb = MultiLabelBinarizer()
train_label = mlb.fit_transform(train_labels)

# Turn the list of binary classifier into a list of np array to stack
train_label_encoded = []
for i in range(len(train_label)):
    x = np.expand_dims(train_label[i], axis = 0)
    train_label_encoded.append(x)
    
# Stacking the np array for training
train_label_encoded = np.vstack(train_label_encoded)

# Save the stacked label
np.save('train_label_encoded', train_label_encoded)
...
for i in tqdm(range(len(test))):
    #try:
        ...
        predictions[test['imageId'][i]] = prediction
        #prediction = np.squeeze(prediction, axis=0)
        '''for i, predicted in enumerate(prediction[0]):
            if prediction[0][i] > 0:
                prediction[0][i] = 1
            else:
                prediction[0][i] = 0
        prediction = mlb.inverse_transform(prediction)
        test['Prediction'][i] = prediction'''
...
df['label_id'][0].tolist()

df['label_id'] = ast.literal_eval(df['label_id'])
x = ast.literal_eval(x)

for i in tqdm(range(len(df)):
    try:
        df.at[i, 'label_id'] = ast.literal_eval(df['label_id'][i])
    except:
        print(df['label_id'][i])
